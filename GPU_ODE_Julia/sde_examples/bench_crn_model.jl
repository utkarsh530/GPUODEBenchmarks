...
if !isinteractive()
    open(joinpath(dirname(dirname(@__DIR__)), "data", "SDE", "CRN",
...
data = @benchmark solve($eprob, EM(), EnsembleThreads(), dt = 0.1f0, adaptive = false,
                        save_everystep = false, trajectories = numberOfParameters)
...
if !isinteractive()
    open(joinpath(dirname(dirname(@__DIR__)), "data", "CPU", "SDE", "CRN",
                  "Julia_times_unadaptive.txt"), "a+") do io
        println(io, numberOfParameters, " ", minimum(data.times) / 1e6)
    end
end
