...
data = @benchmark CUDA.@sync DiffEqGPU.vectorized_map_solve($probs, RK4(),
...
if !isinteractive()
    open(joinpath(dirname(@__DIR__), "data", "EnsembleGPUArray",
                  "Julia_EnGPUArray_times_unadaptive.txt"), "a+") do io
...
data = @benchmark CUDA.@sync DiffEqGPU.vectorized_map_solve($probs, Tsit5(),
                                                            EnsembleGPUArray(0.0), $batch,
                                                            true, dt = 0.001f0,
                                                            save_everystep = false,
                                                            dense = false)
