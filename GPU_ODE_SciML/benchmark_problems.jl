...
numberOfParameters_array = [8*4^i for i = 0:3]

data_times = []
for numberOfParameters in numberOfParameters_array
    @info numberOfParameters
    data = @benchmark solve($ensembleProb, Tsit5(), EnsembleThreads(), trajectories = numberOfParameters,
    ...
    data = @benchmark CUDA.@sync solve($ensembleProb, GPUTsit5(), EnsembleGPUKernel(),
                                trajectories = numberOfParameters, adaptive = false, save_everystep = false,
                                dt = dt)
