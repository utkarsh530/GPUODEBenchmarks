...
# data = @benchmark solve($ensembleProb, EM(), EnsembleThreads(), dt = Float64(1 // 2^8), adaptive = false, save_everystep = false, trajectories = numberOfParameters)

if !isinteractive()
    open(joinpath(dirname(dirname(@__DIR__)), "data", "CPU/SDE/times_unadaptive.txt"), "a+") do io
